...
class DepTree(Tree):
    ...
    def render_label(
        ...
        if node._allow_expand:
            ...
            if node_label.plain == "root" or node.is_root:
                icon_exp = "🌳 "
...
class TreeApp(App):
    def __init__(self, subprojects):
        ...
        self.src = subprojects
        super().__init__()
    
    def dir_tree(self, tree: DepTree, rootdir: pathlib.Path, node: TreeNode | None = None):
        for path in pathlib.Path(rootdir).iterdir():
            pot_proj = path.parts[-1]
            if path.is_dir():
                if not node:
                    nod = tree.root.add(pot_proj, expand=False, allow_expand=True)
                else:
                    nod = node.add(pot_proj, expand=False, allow_expand=True)
                self.dir_tree(tree, path, nod)
        return tree
        
    def compose(self) -> ComposeResult:
        with Horizontal(id="parent_container"):
            tree: DepTree[dict] = DepTree(self.src.parts[-1])
            tree.root.expand()
            self.dir_tree(tree, self.src)
...
if __name__ == "__main__":
    app = TreeApp(pathlib.Path('monorepo-sample'))
    print(app.run())